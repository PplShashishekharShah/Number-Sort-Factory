...
// Number Sort Factory — educational math sorting game.
...
export default function GameContainer() {
    ...
    const onUp = (e) => {
      ...
      if (droppedCatId) {
        ...
        if (ANSWERS[word.id].includes(droppedCatId)) {
          ...
          setFlashType('wrong'); setRobotFeedback(`Oops! ${word.text}: ${hint} Try again!`)
  ...
  useEffect(() => {
    if (Object.keys(sortedWords).length === QUESTION.words.length && QUESTION.words.length > 0) {
      ...
      setRobotFeedback(isFinalLevel ? "MISSION COMPLETE! All levels finished! You're a Number Sort Master! 🏆" : "LEVEL COMPLETE! Moving to the next challenge... 🚀")
  ...
  const restart = () => { 
    ...
    setGameOver(false); setIsPaused(false); setIsCelebrating(false); setRobotFeedback("Welcome back! Let's sort numbers at the Factory! 🏭")
    setSpawnQueue(INITIAL_QUEUE.map(w => w.id)); setRetryQueue([]); setBeltWords([]); setWordPositions({})
    if (!isMuted) speak("Game restarted! Let's crunch some numbers!")
  }
      ...
      <div style={{
        ...
        {activeHint ? (
          <div style={{ animation: 'hint-flicker 0.2s infinite' }}>
            <h4 style={{ margin: '0 0 5px 0', color: 'var(--accent-cyan)', fontSize: 20, fontWeight: 900, letterSpacing: 2 }}>{activeHint.wordText}</h4>
            <p style={{ margin: 0, color: '#fff', fontSize: 12, fontWeight: 600, lineHeight: 1.3 }}>{activeHint.hintText}</p>
          </div>
        ) : (
          <div style={{ opacity: 0.4, color: 'var(--accent-cyan)', fontSize: 12, fontWeight: 900, letterSpacing: 2 }}>NUMBER SORT FACTORY</div>
        )}
      </div>
      ...
      {isCelebrating && (
        <div style={{ position: 'absolute', top: '35%', left: 0, right: 0, display: 'flex', justifyContent: 'center', zIndex: 6000, pointerEvents: 'none' }}>
          <div className="glass" style={{ border: '4px solid var(--accent-cyan)', borderRadius: 40, padding: '30px 100px', animation: 'banner-pop 0.6s cubic-bezier(0.175, 0.885, 0.32, 1.275) forwards' }}>
            ...
            <p style={{ color: '#fff', textAlign: 'center', fontSize: 20, margin: '10px 0 0 0', fontWeight: 600 }}>{QUESTION.theme} — FACTORY APPROVED! 🏭</p>
          </div>
        </div>
      )}
